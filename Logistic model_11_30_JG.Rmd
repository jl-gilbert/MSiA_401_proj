...
```{r}
model_3 <- glm(targdol ~.-recency_year-ordhist,family=binomial(link='logit'),data=train_2)
summary(model_3)
...
```
```{r}
model.final.2 <- glm(formula = targdol ~ sls3ago + slshist + ordtyr + ordlyr + 
...
```{r}
model.full<-model_3
anova(model.full,model.final.2,test="Chisq")
```
...
```
```{r}
model.final.3 <- glm(formula = targdol ~ sls3ago + slshist + ordtyr + ordlyr + 
    ordhist + recentseason + recency_year + lpmonth + 
    slshist_0.5 + ordtyr_2 + sprord_0.5 + years_since_purchase_0.5 + 
    slstyr_slslyr_sls2ago_sls3ago + slstyr_sls3ago + slstyr_slshist + 
    sls2ago_slshist + slstyr_greater_slslyr + consistent3 + consistent4 + 
    consistent7 + consistent9 + consistent10 + consistent11, 
    family = binomial(link = "logit"), data = train_2)
summary(model.final.3)
vif(model.final.3)
```
