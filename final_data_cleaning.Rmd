...
if (df$year_lp6yr == df$year_ordyr) {
  
} else if (df$year_lp6yr > df$year_ordyr) {
...
df$recency_year <- df$max_year[df$max_year >= 2009]
df$recency_year <- df$year[df$max_year < 2009]

table(df$max_year, df$year)
```
